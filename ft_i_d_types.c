...
/*   Updated: 2020/07/28 12:58:48 by kbatwoma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_pr(int precision, int number, char **str, \
								size_t len_str)
{
	int		index;
	char	*next_str;
...
char		*ft_i_d_types(t_parser *flags, va_list arg)
{
	...
	flag = flags->flag;
	n = va_arg(arg, int);
	if ((str = ft_itoa(n)) == NULL)
		return (NULL);
	if (n == 0 && flags->precision == 0)
		str[0] = '\0';
	else if (flags->precision > ft_strlen(str) || \
			(flags->precision == ft_strlen(str) && n < 0))
		if ((str = ft_pr(flags->precision, n, &str, ft_strlen(str))) == NULL)
			return (NULL);
	...
	else
	{
		if (flags->pr_for_s == 1 && flags->width > flags->precision && \
			flags->flag != 0)
			flag = 1;
		str = ft_width(&str, flags->width, flag, n);
	}
	return (str);
}
